...
const roomTags = computed(() =>
  props?.chat?.messages.length
    ...
    : props?.chat?.initialRoomTags
      ? props.chat.initialRoomTags
      : []
);
const roomPubkeys = computed(() =>
  roomTags.value.length ? [...new Set(roomTags.value.map((t) => t[1]))] : []
);
const roomPubkeysWihoutUser = computed(() =>
  roomPubkeys.value.filter((pubkey) => pubkey !== props.userPubkey)
);
...
onUpdated(async () => {
  ...
  const metasDM = await pool.querySync(relays, {
    kinds: [EVENT_KIND.DM_RELAYS],
    authors: pubkeysToDownload,
  });
  ...
  metasDM.forEach((event: Event) => {
    const hasRelay = event.tags.some(
      (tag) => (tag[0] === 'r' || tag[0] === 'relay') && tag[1] && tag[1].length
    );
    if (hasRelay) {
  ...
  if (pubkeysToDownload.length) {
    const metas = await pool.querySync(relays, {
...
<template>
  <div :class="['conversation', { 'conversation_mobile-hidden': showChatsList }]">
    ...
    <div v-if="chat" class="chats__messages" ref="messagesContainer">
      <ChatMessagesList :messages="chat.messages" :userPubkey="userPubkey" />
    </div>
