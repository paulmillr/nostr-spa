...
<template>
  <div class="event-details">
    <div :class="['event-details__header', { 'event-details__header_user': isUserEvent }]">
      <span
        @click="() => handleRawDataTabClick(1)"
        :class="['event-details__tab', { 'event-details__tab_active': rawDataActiveTab === 1 }]"
        >Note</span
      >
      <span
        v-if="showAuthorTab"
        @click="() => handleRawDataTabClick(2)"
        :class="['event-details__tab', { 'event-details__tab_active': rawDataActiveTab === 2 }]"
        >Author</span
      >
      <span
        @click="() => handleRawDataTabClick(3)"
        :class="['event-details__tab', { 'event-details__tab_active': rawDataActiveTab === 3 }]"
        >Author content</span
      >
    </div>
    ...
    <div v-if="showAuthorTab && rawDataActiveTab === 2">
      <pre v-if="authorEvent" class="highlight">{{
        JSON.stringify(clearAuthorEvent, null, 2)
      }}</pre>
